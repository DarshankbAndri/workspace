    ...
    getDowntimeEntries()
      .then((data) => {
        ...
        const grouped = data.reduce((acc, item) => {
          ...
          const current = acc[key] || { id: key, equipmentName: item.equipmentName, events: 0, totalHours: 0, plannedEvents: 0 };
          current.events += 1;
          current.totalHours += Number(item.downtimeHours || 0);
          current.plannedEvents += item.planned ? 1 : 0;
          acc[key] = current;
          return acc;
        }, {});
        setSummaryRows(Object.values(grouped).map((item) => ({ ...item, totalHours: item.totalHours.toFixed(2) })));
      })
      .catch(() => setError('Unable to load downtime analysis.'))
      .finally(() => setLoading(false));
  }, []);

  const totalHours = rows.reduce((sum, item) => sum + Number(item.downtimeHours || 0), 0).toFixed(2);
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        ...
        <Grid item xs={12} md={4}><Paper sx={{ p: 2, borderRadius: 1 }}><Typography variant="body2" color="text.secondary">Total Hours</Typography><Typography variant="h4" fontWeight={800}>{totalHours}</Typography></Paper></Grid>
      </Grid>
